...
def get_optimizer(model, config, model_type):
    ...
    Args:
        ...
        model_type (str): Type of model ('transfer' or 'scratch').
    ...
    if model_type == "transfer":
        # Finetuning mode - only optimize trainable parameters
        ...
        # From scratch mode - optimize all parameters
...
class BalancedSampler(Sampler):
    ...
    Args:
        ...
        samples_per_class (int, optional): Number of samples to use per class.
            If None, uses the size of the smallest class.
    """

    def __init__(self, labels, samples_per_class=None):
        self.labels = labels
        self.class_to_indices = self._group_indices_by_class()
        self.samples_per_class = samples_per_class or min(
            len(v) for v in self.class_to_indices.values()
        )
    ...
    def __iter__(self):
        ...
        Returns:
            iterator: Iterator over shuffled balanced indices.
        """
        indices = []
    ...
    def __len__(self):
        """Return the total number of samples that will be drawn.
...
def train(config):
    ...
    # Initialize data loaders with balanced sampling
    sampler = BalancedSampler(
        labels_train,
        samples_per_class=min(Counter(labels_train).values()) * 2
    )
    ...
    with mlflow.start_run():
        ...
        for epoch in range(1, config["training"]["epochs"] + 1):
            ...
            # Log first epoch class distribution
            if epoch == 1:
